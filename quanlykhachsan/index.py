...
@app.route('/yeuCauDatPhong', methods=['get', 'post'])
def yeucau():
    err_msg = ''
    flag = 0
    date_format = "%Y-%m-%d"
...
@app.route('/chiTietPhieuDatRong/<int:phong_id>', methods=['get', 'post'])
@login_required
...
@app.route('/chiTietPhieuDat/<int:phieudatphong_id>', methods=['get', 'post'])
@login_required
...
def thuephong():
    ...
    return render_template('thuePhong.html', phieuthuephong=phieuthuephong, phieuthuechitiet=phieuthuechitiet,
                           khachhang=khachhang, date=date_pt)


# gọi trang Chi tiết phiếu thue
# ----- 1. Từ sơ đồ phòng: rỗng
@app.route('/chiTietPhieuThueRong/<int:phong_id>', methods=['get', 'post'])
@login_required
...
# ----- 2.Từ dsach đặt
@app.route('/chiTietPhieuThueDatTruoc/<int:phieudatphong_id>', methods=['get', 'post'])
@login_required
...
@app.route('/chiTietPhieuThue_HD/<int:phieuthuephong_id>', methods=['get', 'post'])
@login_required
